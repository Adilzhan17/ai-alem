...
const roleOptions: Array<{ id: UserRole; label: string; icon: string; hint: string }> = [
  { id: 'client', label: 'Застройщик / Заказчик', icon: 'apartment', hint: 'Проекты, сметы и управление исполнителями' },
  { id: 'contractor', label: 'Подрядчик', icon: 'engineering', hint: 'Работы, сроки и предложения по RFQ' },
  { id: 'supplier-materials', label: 'Поставщик', icon: 'inventory_2', hint: 'Материалы, каталог и ответы на запросы' }
]
        ...
        <main className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="magic-card rounded-3xl p-6 md:p-8">
            ...
            <p className="mt-4 max-w-lg text-sm leading-relaxed text-qal-text-secondary md:text-base">
              Выберите роль, и система сразу откроет профильный кабинет после создания аккаунта.
            </p>
          ...
          <section className="magic-card rounded-3xl p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-qal-text-secondary">Заполните базовые данные для старта.</p>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              <div>
                <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.14em] text-qal-text-secondary">Полное имя</label>
                <input
                  required
                  value={fullName}
              ...
              {role !== 'client' && (
                <div>
                  <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.14em] text-qal-text-secondary">Компания</label>
                  <input
                    required
                    value={companyName}
            ...
            <p className="mt-6 text-sm text-qal-text-secondary">
              ...
              <Link to="/login" className="font-semibold text-qal-primary hover:text-qal-primary-dark">
                Войти
              </Link>
            </p>
          </section>
